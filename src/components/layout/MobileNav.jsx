 ...
 * Navegación en móvil.
 *
 * Nueve secciones no caben en una fila de pestañas sin convertirse en un carrusel
  ...
  return (
    ...
    >
      <nav className="-mx-2 flex flex-col gap-1">
        {VIEWS.map(({ id, label, icon: Icon, subtitle, apart }) => {
          const active = id === view
          return (
            <Fragment key={id}>
              {/* Las notas no son una sección de cifras más: la línea lo dice. */}
              {apart ? <hr className="my-2 border-ink-100" /> : null}
              <button
                onClick={() => {
                  onChange(id)
                  onClose()
